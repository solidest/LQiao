...
5. [内置工具](#5-内置工具)
6. [自定义工具](#6-自定义工具)
7. [ReAct Agent 推理循环](#7-react-agent-推理循环)
8. [CodeAgent 代码执行](#8-codeagent-代码执行)
9. [沙箱与安全](#9-沙箱与安全)
10. [事件系统](#10-事件系统)
...
├─────────────────┤
│  ReactAgent     │  ← 推理引擎：Thought → Action → Observation
│  CodeAgent      │  ← 代码执行引擎
├─────────────────┤
│  工具层          │  ← FileTool / GitTool / 自定义工具
├─────────────────┤
│  模型层          │  ← OpenAI / Anthropic 适配器
├─────────────────┤
│  安全层          │  ← 沙箱 / 权限 / 审计
└─────────────────┘
```
...
| `Sandbox` | 沙箱隔离 | 直接控制沙箱时（通常由 Agent 内部管理） |
| `DefaultEventBus` | 事件总线 | 需要监听 Agent 内部事件时 |
...
```typescript
// 从 GPT 切换到 Claude
agent.switchModel('claude-3.7', process.env.ANTHROPIC_API_KEY);
...
| `claude-` | Anthropic | claude-3.5-sonnet, claude-3.7-sonnet, claude-opus-4-1 |
...
```typescript
import { modelRegistry, OpenAIModel } from 'lqiao';

// 注册一个兼容 OpenAI API 的本地模型（如 Ollama）
modelRegistry.registerProvider('llama', {
  provider: 'openai',
...
```typescript
import { Agent, GitTool } from 'lqiao';
...
如果你不需要 `Agent` 主类的聚合能力，可以直接使用 `ReactAgent`：
...
当前 ReAct Agent 使用内置的默认 prompt。如果需要自定义系统提示词，可以直接继承 `ReactAgent`：

```typescript
import { ReactAgent } from 'lqiao';

class CustomReactAgent extends ReactAgent {
  // 当前版本 prompt 模板内嵌在 run() 方法中
  // 自定义 prompt 建议在 fork ReactAgent 源码后修改 DEFAULT_REACT_PROMPT
}
```
...
agent.on(AGENT_EVENTS.beforeRun, (data) => {
  console.log('开始任务:', data.task);
});

agent.on(AGENT_EVENTS.onToolCall, (data) => {
  console.log(`调用工具: ${data.tool}`, data.args);
});

agent.on(AGENT_EVENTS.afterRun, (data) => {
  console.log(`任务完成，共 ${data.steps} 步`);
  console.log('结果:', data.answer);
});

agent.on(AGENT_EVENTS.onError, (data) => {
...
```typescript
agent.once(AGENT_EVENTS.afterRun, (data) => {
  // 只触发一次，之后自动移除监听
...
profiler.end('model-call');

profiler.start('tool-call');
await fileTool.execute('read', '/path');
profiler.end('tool-call');

// 查看结果
console.log(profiler.getTimings());
// [
//   { name: 'model-call', duration: 234 },
//   { name: 'tool-call', duration: 5 },
// ]

console.log(profiler.getAverage()); // 平均耗时
console.log(profiler.getTotal());   // 总耗时
```
...
| `NETWORK_ERROR` | `ERROR_TYPES.NETWORK_ERROR` | 网络错误 | 是 |
| `RATE_LIMIT_ERROR` | `ERROR_TYPES.RATE_LIMIT_ERROR` | 限流 | 是 |
...
**Agent 执行流程：**
```
Step 1: Thought → 我需要先读取文件
      Action: file → read src/utils.ts
      Observation: 文件内容 "{...}"

Step 2: Thought → 我发现了一个 bug，需要修复
      Action: file → write src/utils.ts
      Observation: 写入成功

Step 3: Thought → 现在需要提交修改
      Action: git → add src/utils.ts
      Observation: 添加成功

Step 4: Action: git → commit
      Observation: 提交成功

Step 5: Action: git → push
      Observation: 推送成功

Final Answer: Bug 已修复并提交到远程仓库
```

### 15.2 场景二：代码生成（CodeAgent）
...
// { success: true, output: 3 }  (7 在数组中的索引)
```
...
// 生成 React 组件
const component = await agent.run(`
  创建一个 TypeScript React 组件 UserCard.tsx：
  - 接收 name、email、avatarUrl 三个 props
  - 使用 Tailwind CSS 样式
  - 包含加载状态
  将代码写入 src/components/UserCard.tsx
`);

// 修复 ESLint 错误
const fixResult = await agent.run(`
  读取 src/components/UserCard.tsx，
  检查并修复潜在的 ESLint 问题：
  - 未使用的变量
  - 缺失的类型注解
  - 未处理的 Promise
...
// 任务开始前记录
agent.on(AGENT_EVENTS.beforeRun, () => {
  console.time('task');
});
...
```typescript
import { modelRegistry, OpenAIModel, Agent } from 'lqiao';

// 注册 Ollama 提供者（复用 OpenAI 兼容协议）
modelRegistry.registerProvider('llama', {
  provider: 'openai',
  baseUrl: 'http://localhost:11434/v1',
});
...
3. 指定工作目录：`new GitTool(undefined, 'C:\\my\\project')`
